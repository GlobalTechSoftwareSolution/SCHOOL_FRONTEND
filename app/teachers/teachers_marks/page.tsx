...
export default function MarksManager() {
  ...
  const grouped = useMemo(() => {
    ...
    students.forEach((s) => {
      ...
      const sectionName = s.section || s.student_section || idxClass[cid]?.section || "Default";
  ...
  const toggleSection = (classId: number, sectionName: string) => {
  ...
  }, [students, grades, grouped, assignedClassIds, reportType, selectedClassId, selectedSectionName]);
  ...
  const calculateStudentAverage = (email: string) => {
  ...
  function cancelEdit(gradeId: number) {
    setEditing((p) => {
      const copy = { ...p };
      delete copy[gradeId];
      return copy;
  ...
  async function saveEdit(grade: Grade) {
    ...
    try {
      ...
      addNotification("Update Failed", "Failed to save marks: " + (err.message || "Unknown error"), "error");
    } finally {
      setSaving((s) => {
        const copy = { ...s };
        delete copy[id];
  ...
  function openSendModal(classId: number, sectionName: string | null) {
  ...
  useEffect(() => {
    ...
    sectionStudents.forEach((s: Student) => {
      let p = s.parent;
      if (!p && typeof s.parent === "object" && s.parent?.email) p = s.parent.email;
      po[s.id] = p || "";
    });
  ...
  function setParentForStudent(studentId: number, email: string) {
    setParentOverrides((p) => ({ ...p, [studentId]: email }));
  }

  // send reports to selected parents
  async function sendReportToParents() {
    ...
    try {
      const tasks = selectedStudentsForSend.map(async (studentId) => {
        ...
        if (!parentEmail) {
          let p = st.parent;
          if (p && typeof p === "object") parentEmail = p.email || "";
          else parentEmail = p || "";
        }
        
        const body = {
          email: st.email || st.student_email || st.email_address || '',
          parent_email: parentEmail,
          report_type: reportType,
          from_teacher: teacherEmail,
          student_id: st.student_id || st.id || '',
          student_name: st.name || st.fullname || st.first_name || ''
        };
      ...
      addNotification("Send Failed", "Failed to send reports: " + (err.message || "Unknown error"), "error");
    } finally {
      setSending(false);
    }
  }
  ...
  // -------------------- small helpers --------------------
  function studentParentEmail(student: Student) {
    if (!student) return "";
    if (!student.parent) return "";
    if (typeof student.parent === "string") return student.parent;
    if (typeof student.parent === "object" && student.parent.email) return student.parent.email;
    return "";
  }
                                    ...
                                    <div key={s.id || s.email || s.student_id || `student-${index}`} className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition-all duration-200 bg-white">
                                      ...
                                      <div className="flex items-start justify-between mb-4">
                                        ...
                                        <div className="text-right">
                                          <div className={`text-lg font-bold ${gradeColor.split(' ')[0]}`}>
                                            {studentAvg}%
                                          </div>
                                      ...
                                      <div className="mb-4">
                                        <div className="flex items-center justify-between text-xs mb-2">
                                          ...
                                          <span className={`font-medium ${gradeColor.split(' ')[0]}`}>
                                            {studentAvg >= 80 ? 'Excellent' : studentAvg >= 60 ? 'Good' : 'Needs Improvement'}
                                          </span>
                                        </div>
                                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                          <div 
                                            className={`h-full rounded-full ${gradeColor.split(' ')[0].replace('text-', 'bg-')}`}
                                      ...
                                      <div className="flex gap-2 mb-4">
                                        ...
                                        <button
                                          onClick={async () => {
                                            ...
                                            let parentEmail = studentParentEmail(s);
                                            ...
                                            try {
                                              const body = {
                                                email: s.email || s.student_email || s.email_address || '',
                                                parent_email: parentEmail,
                                                report_type: reportType,
                                                from_teacher: teacherEmail,
                                                student_id: s.student_id || s.id || '',
                                                student_name: s.name || s.fullname || s.first_name || ''
                                              };
                                              ...
                                              addNotification("Send Failed", `Failed to send report: ${err.message}`, "error");
                                            }
                                          }}
