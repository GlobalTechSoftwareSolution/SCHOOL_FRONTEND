  ...
  useEffect(() => {
    const loadAttendance = async () => {
      ...
      try {
        ...
        if (mode === "students") {
          normalizedAttendance = (attRes.data || []).map((item: any) => ({
            ...item,
            role: "student",
  ...
  const getDisplayName = (row: any) => {
    ...
    if (isStudent) {
      ...
      if (resolved && resolved.student) {
        return resolved.student.fullname || resolved.student.name || row.user_name || row.user_email;
      }
    }
