      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/20 to-purple-50/10 p-4 sm:p-6">
        <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            <div className="bg-gradient-to-br from-white to-blue-50/50 rounded-xl sm:rounded-2xl shadow-sm border border-blue-200/30 p-3 sm:p-4 lg:p-6 relative overflow-hidden group hover:shadow-md transition-all duration-300 col-span-1 xs:col-span-2 lg:col-span-1">
                ...
                <div className="flex flex-wrap gap-1 sm:gap-2 bg-gray-100 p-1 rounded-lg sm:rounded-xl">
                  {[
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-3 py-1 sm:py-2 rounded-md sm:rounded-lg font-medium transition-all duration-300 text-xs sm:text-sm ${activeFilter === filter.id
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
              <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-2xl max-h-[95vh] overflow-y-auto border border-gray-200/60">
                ...
                <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <InfoRow
                          label="Important"
                          value={
                            <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-semibold ${selectedNotice.important
                  ...
                  {/* Actions - Only Close button now as Share/Download removed */}
...
// Enhanced Notice Card Component with Professional UI
const NoticeCard = ({ notice, onView, onDelete, getTypeStyles, getTypeIcon }: NoticeCardProps) => (
  <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 group flex flex-col h-full overflow-hidden">
    <div className="p-4 sm:p-5 flex flex-col h-full">
      <div className="flex items-start justify-between mb-3">
        <div className="flex flex-wrap gap-2 mb-2">
          <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${getTypeStyles(notice.type || 'info')}`}>
            {getTypeIcon(notice.type || 'info')}
            <span className="capitalize">{notice.type || 'info'}</span>
          </span>
          {notice.important && (
      ...
      <div className="pt-4 border-t border-gray-50 mt-auto">
        <div className="flex items-center justify-between text-xs text-gray-500">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1.5" title="Posted Date">
              <Clock className="h-3.5 w-3.5 text-gray-400" />
              {new Date(notice.posted_date || '').toLocaleDateString()}
            </span>
            <span className="flex items-center gap-1.5" title="Author">
              <User className="h-3.5 w-3.5 text-gray-400" />
              <span className="truncate max-w-[80px]">{notice.notice_by ? notice.notice_by.split('@')[0] : 'System'}</span>
            </span>
          </div>
        </div>
      </div>
    ...
    <div className="sm:hidden grid grid-cols-2 border-t border-gray-100 divide-x divide-gray-100 bg-gray-50/50">
      <button
        onClick={onView}
        className="py-3 text-sm font-medium text-blue-600 hover:bg-blue-50 transition-colors flex items-center justify-center gap-2"
      >
        <Eye className="h-4 w-4" />
        View
      </button>
      <button
        onClick={onDelete}
        className="py-3 text-sm font-medium text-red-600 hover:bg-red-50 transition-colors flex items-center justify-center gap-2"
      >
        <Trash2 className="h-4 w-4" />
        Delete
      </button>
