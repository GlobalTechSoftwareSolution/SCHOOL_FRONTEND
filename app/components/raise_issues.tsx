...
const Issues_Page = () => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case "Closed": return <FiCheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />;
      case "In Progress": return <FiClock className="w-3 h-3 sm:w-4 sm:h-4" />;
      case "Open": return <FiAlertTriangle className="w-3 h-3 sm:w-4 sm:h-4" />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-4 sm:p-6">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div className="text-center lg:text-left">
            ...
            <p className="text-gray-600 mt-2 text-sm sm:text-base">Track and resolve system issues efficiently</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-4 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl flex items-center justify-center gap-2 w-full xs:w-auto text-sm sm:text-base"
          >
            {showForm ? <FiX className="w-4 h-4 sm:w-5 sm:h-5" /> : <FiPlus className="w-4 h-4 sm:w-5 sm:h-5" />}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-gray-200 col-span-2 lg:col-span-1">
          <div className="flex items-center justify-between">
            ...
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-blue-50 rounded-lg sm:rounded-xl flex items-center justify-center">
              <FiArchive className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-l-4 border-l-red-500">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-lg sm:text-xl font-bold text-gray-900">{stats.open}</div>
        ...
        <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-l-4 border-l-blue-500">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-lg sm:text-xl font-bold text-gray-900">{stats.inProgress}</div>
        ...
        <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-l-4 border-l-green-500">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-lg sm:text-xl font-bold text-gray-900">{stats.closed}</div>
        ...
        <div className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl shadow-sm border border-l-4 border-l-orange-500">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-lg sm:text-xl font-bold text-gray-900">{stats.highPriority}</div>
              <div className="text-gray-600 text-xs sm:text-sm mt-1">High Priority</div>
            </div>
            <div className="w-8 h-8 sm:w-10 sm:h-10 bg-orange-50 rounded-lg sm:rounded-xl flex items-center justify-center">
      ...
      {showForm && (
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg border border-gray-200 p-4 sm:p-6 mb-6 sm:mb-8">
          ...
          <form onSubmit={handleAddIssue} className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="md:col-span-2 flex flex-col xs:flex-row gap-2 sm:gap-3 justify-end">
              <button
                type="button"
                onClick={() => setShowForm(false)}
                className="px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 text-gray-700 rounded-lg sm:rounded-xl hover:bg-gray-50 transition-colors font-medium text-sm sm:text-base order-2 xs:order-1"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-colors flex items-center justify-center gap-2 text-sm sm:text-base order-1 xs:order-2"
              >
        ...
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-3 border border-gray-300 rounded-lg sm:rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm sm:text-base"
            />
          </div>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 p-4 sm:p-6">
            {filteredIssues.map((issue) => (
              <div key={issue.id} className="border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 hover:shadow-lg transition-all duration-200 bg-white">
                {/* Header */}
                <div className="flex justify-between items-start mb-3 sm:mb-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getPriorityColor(issue.priority)}`}>
                        {issue.priority}
                      </span>
                      <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(issue.status)} flex items-center gap-1`}>
                ...
                <div className="space-y-2 mb-4 sm:mb-6">
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                    ...
                    <span className="truncate">By: {issue.raised_by || "—"}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                    <FiUser className="w-3 h-3 sm:w-4 sm:h-4" />
                    <span className="truncate">To: {issue.raised_to || "—"}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                ...
                <div className="flex justify-between items-center pt-3 sm:pt-4 border-t border-gray-100">
                  ...
                  <div className="flex gap-1 sm:gap-2">
                    <button
                      ...
                      className="bg-blue-50 hover:bg-blue-100 text-blue-700 p-1 sm:p-2 rounded-lg transition-colors"
                      title="View Details"
                    >
                    ...
                    {issue.status !== "Closed" && (
                      <button
                        ...
                        className="bg-green-50 hover:bg-green-100 text-green-700 p-1 sm:p-2 rounded-lg transition-colors"
                        title="Close Issue"
                      >
                    ...
                    {issue.status === "Open" && (
                      <button
                        ...
                        className="bg-orange-50 hover:bg-orange-100 text-orange-700 p-1 sm:p-2 rounded-lg transition-colors"
                ...
                <div className="mt-3 sm:hidden">
                  <select
                    ...
                    className={`w-full px-3 py-2 rounded-lg text-sm font-medium border focus:outline-none ${getStatusColor(issue.status)}`}
                  >
      ...
      {showView && selectedIssue && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-3 sm:p-4">
          <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl w-full max-w-4xl max-h-[95vh] overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-4 sm:p-6 text-white">
              <div className="flex justify-between items-start">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-white bg-opacity-20 ${getPriorityColor(selectedIssue.priority)}`}>
                      {selectedIssue.priority} Priority
                    </span>
                    <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-white bg-opacity-20 ${getStatusColor(selectedIssue.status)} flex items-center gap-1`}>
                ...
                <button
                  ...
                  className="text-white hover:text-gray-200 transition-colors ml-2 sm:ml-4 flex-shrink-0"
                >
                  ...
                  <div>
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      ...
                      <button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-colors flex items-center justify-center gap-2 w-full sm:w-auto text-sm sm:text-base"
                      >
            ...
            <div className="border-t border-gray-200 p-4 sm:p-6 bg-gray-50">
              <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 justify-between">
                <div className="flex flex-col xs:flex-row gap-2 sm:gap-3">
                  {selectedIssue.status !== "Closed" && (
                    <button
                      ...
                      className="bg-green-600 hover:bg-green-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-colors flex items-center justify-center gap-2 text-sm sm:text-base flex-1"
                    >
                      <FiCheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />
                      Close Issue
                    </button>
                  ...
                  {selectedIssue.status === "Open" && (
                    <button
                      ...
                      className="bg-orange-600 hover:bg-orange-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-colors flex items-center justify-center gap-2 text-sm sm:text-base flex-1"
                    >
                      <FiClock className="w-3 h-3 sm:w-4 sm:h-4" />
                      Mark In Progress
                    </button>
                ...
                <button
                  ...
                  className="bg-red-600 hover:bg-red-700 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl font-medium transition-colors flex items-center justify-center gap-2 text-sm sm:text-base flex-1 sm:flex-none"
                >
                  <FiTrash2 className="w-3 h-3 sm:w-4 sm:h-4" />
                  Delete Issue
                </button>
